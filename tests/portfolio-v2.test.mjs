...
test("contact remains direct-link only", () => { const contact = read("src/app/contact/page.tsx"); assert.match(contact, /mailto:/); assert.doesNotMatch(contact, /ContactForm|api\/contact|<form/i); });
test("no orange-led palette or FastAPI remains in visible V2 source", () => { const files = ["src/data/config.ts", "src/app/globals.css", "src/app/page.tsx", "src/app/about/page.tsx", "src/app/portfolio/page.tsx", "src/app/portfolio/[slug]/page.tsx"]; const source = files.map(read).join("\n"); assert.doesNotMatch(source, /#FF9900|#ff9900|FastAPI/); });
